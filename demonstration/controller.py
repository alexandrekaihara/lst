...
class Controller(Node):
    ...
    def initController(self, ip:str, port: int, command=[]):
        try:
            if len(command) == 0:
                subprocess.run(f"docker exec {self.getNodeName()} ryu-manager --ofp-listen-host={ip} --ofp-tcp-listen-port={port} /home/controller.py > controller.log 2>&1 &", shell=True)
            else:
