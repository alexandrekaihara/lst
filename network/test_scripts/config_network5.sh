...
## Create flows
ovs-ofctl add-flow br-int priority=500,ip,nw_dst=192.168.100.0/27,actions=output:1
ovs-ofctl add-flow br-int priority=500,ip,nw_dst=192.168.200.0/27,actions=output:2
ovs-ofctl add-flow br-int priority=500,ip,nw_dst=192.168.210.0/27,actions=output:3
ovs-ofctl add-flow br-int priority=500,ip,nw_dst=192.168.220.0/27,actions=output:4
...
create_subnet(){
    ...
    until docker network create -d macvlan --subnet=192.168.$1.0/24 --ip-range=192.168.$1.0/27 -o parent=eth$1.$1 -o macvlan_mode=bridge --aux-address="host=192.168.$1.30" cidds$1
    do 
